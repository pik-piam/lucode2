...
#' @return Invisibly, a named logical vector indicating whether a package requirement is met.
...
checkRequirement <- function(package, version) {
  requiredVersion <- as.package_version(if (version == "*") "0.0" else gsub("[^0-9.]", "", version))
  packageVersion <- if (package == "R") {
    as.package_version(paste(R.version[c("major", "minor")], collapse = "."))
  } else {
    tryCatch({
