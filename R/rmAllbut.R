...
#' @param list List specifying the objects to be kept. Same as in \code{\link[base]{rm}}.
#' @param clean Boolean to specify if a \code{\link[base]{gc}} shall be executed
...
rmAllbut <- function(..., list = character(), clean = TRUE) {
  ...
  if (length(keepObjects) == 0) {
    stop("No argument to rmAllbut specified. Use rm(list=ls()) instead")
  }
