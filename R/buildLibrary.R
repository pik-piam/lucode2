...
buildLibrary <- function(lib = ".", cran = TRUE, updateType = NULL, gitpush = FALSE, commitmessage = NULL, # nolint
  ...
  ###################################################################
  gitignore <- readLines(file.path(lib, ".gitignore"))
  if ("*.Rd" %in% gitignore || file.exists(file.path(lib, "man", ".gitignore"))) {
    ...
    if (file.exists(file.path(lib, "man", ".gitignore"))) {
      message("removing man/.gitignore")
      file.remove(file.path(lib, "man", ".gitignore"))
    }
  }
