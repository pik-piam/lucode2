...
#' @param installFunction Optional function. Will be called during checkRequiredPackages like this:
#' installFunction(missingPackages). Only needed if the required packages are not available on CRAN or the configured
...
checkRequiredPackages <- function(requiredPackages, requiredFor = "", installFunction = install.packages) {
  ...
  question <- paste0("The following currently not installed package",
                     ...
                     " now? (y/N)")
  if (tolower(readline(question)) %in% c("y", "yes")) {
    installFunction(missingPackages)
  }
  ...
  if (length(stillMissingPackages) > 0) {
    stop(paste0("The following package",
                ...
                if (length(missingPackages) == 1) "it" else "they",
